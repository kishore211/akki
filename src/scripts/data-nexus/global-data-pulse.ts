...
export const initGlobalDataPulse = async () => {
  const roots = Array.from(document.querySelectorAll<HTMLElement>('[data-global-pulse]'));

  await Promise.all(
    roots.map(async (root) => {
      if (root.dataset.globalPulseReady === 'true') return;
      ...
      if (!canvas || !tooltip || regions.length === 0) return;
      ...
      const showRegion = (region: PulseRegion, x = 16, y = 16) => {
        buttons.forEach((button) => button.classList.toggle('is-active', button.dataset.regionId === region.id));
        tooltip.innerHTML = `
      ...
      canvas.addEventListener('pointermove', (event) => {
        ...
        if (region) showRegion(region, event.offsetX + 12, event.offsetY + 12);
      });
      ...
      buttons.forEach((button) => {
        button.addEventListener('click', () => {
          ...
          if (region) showRegion(region, 16, 16);
        });
      });
